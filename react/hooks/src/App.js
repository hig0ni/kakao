...
class ClassEffect extends React.Component{
    ...
    //Component가 Mount 된 후 호출되는 메서드
    componentDidMount(){
        console.log("마운트 된 후 호출되는 메서드")
        document.title = `You clicked ${this.state.count} times`;
    }

    //Component가 Update 된 후 호출되는 메서드
    componentDidUpdate(){
...
const App = () =>{
  ...
  //배열의 데이터를 수정하면 컴퍼넌트가 리랜더링 될 수 있도록
  //state로 생성
  const [users, setUsers] = useState([
      {id:1, username:'adam', email:'it@kakao.com', active:false},
      {id:2, username:'lee', email:'lee@kakao.com', active:false}
  ]);
  ...
  const onRemove = (id)=>{
    ...
    //id가 일치하지 않는 데이터만 가지고 배열을 만들어서 수정
  ...
  //Active가 활성화된 user 개수를 세는 함수 호출
